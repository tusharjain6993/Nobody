  ...
  return (
    <div className="p-6 max-w-[1380px] mx-auto space-y-5">
      <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
        <div>
          <h1 className="text-xl font-extrabold text-slate-900 dark:text-slate-100 mb-1">Admin Calendar</h1>
      ...
      {loading ? (
        <div className="text-sm text-slate-500 py-8 text-center">Loading admin calendar…</div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-5">
          <div className="bg-white rounded-3xl border border-slate-100 shadow-3d overflow-hidden">
                ...
                <div className="grid grid-cols-7 min-h-[520px]">
                  {weekDays.map((day) => (
                    <div key={day.date.toISOString()} className="border-r border-slate-100 p-3 space-y-2">
                      {day.items.length === 0 ? <div className="text-xs text-slate-300">No meetings</div> : day.items.map((item) => <EventPill key={item.id} item={item} onClick={() => openItem(item)} />)}
                    </div>
                  ))}
                </div>
              </div>
            ...
            {view === "day" && (
              <div className="p-4 min-h-[520px]">
                ...
                <div className="space-y-3">
                  {dayItems.length === 0 ? (
                    <div className="text-sm text-slate-400">No meetings for this day.</div>
                  ) : (
                    dayItems.map((item) => (
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-3d p-4">
            <div className="text-xs font-bold uppercase tracking-wide text-slate-500 mb-3">Upcoming Meetings</div>
            <div className="space-y-3">
              {[...normalizedItems].sort((a, b) => a.startDate - b.startDate).slice(0, 12).map((item) => (
                <button key={item.id} type="button" onClick={() => { setCursorDate(startOfDay(item.startDate)); openItem(item); }} className="w-full text-left border-b border-slate-100 pb-3 last:border-b-0">
                  ...
                  <div className="text-xs text-slate-500 mt-1">{new Date(item.startsAt).toLocaleString()}</div>
                </button>
              ))}
            </div>
          </div>
