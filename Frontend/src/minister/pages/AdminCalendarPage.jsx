...
export default function AdminCalendarPage() {
  ...
  return (
    <div className="portal-page">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button 
            ...
            className="flex items-center justify-center p-2 border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors" 
          ...
          <button 
            ...
            className="px-4 py-2 text-sm font-semibold border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors" 
          ...
          <div className="relative">
            <select
              ...
              className="appearance-none px-4 py-2 pr-8 text-sm font-semibold border rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors bg-transparent outline-none cursor-pointer"
              style={{ borderColor: "var(--border-secondary)", color: "var(--text-primary)" }}
            >
              {VIEW_OPTIONS.map((option) => (
                <option key={option} value={option} className="text-slate-900 dark:text-slate-100 bg-white dark:bg-slate-900">
                  {option[0].toUpperCase() + option.slice(1)}
                </option>
              ))}
            </select>
            <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ color: "var(--text-tertiary)" }}><path d="m6 9 6 6 6-6"/></svg>
            </div>
          </div>
      ...
      {/* --- END REFACTORED HEADER UI --- */}

      {error && <div className="portal-alert portal-alert--error mb-4">{error}</div>}
              ...
              <div>
                ...
                <div className="grid grid-cols-7">
                  {monthCells.map((cell) => {
                    ...
                    const isToday = isSameDay(cell.date, new Date());
                    return (
                      <div key={cell.date.toISOString()} className="min-h-[128px] p-2" style={{ borderBottom: "1px solid var(--border-secondary)", borderRight: "1px solid var(--border-secondary)", background: inMonth ? "var(--bg-primary)" : "var(--bg-secondary)" }}>
                        <button type="button" onClick={() => setCursorDate(cell.date)} className={`w-8 h-8 rounded-full text-xs font-semibold ${isToday ? "bg-blue-600 text-white" : ""}`} style={!isToday ? { color: inMonth ? "var(--text-primary)" : "var(--text-tertiary)" } : undefined}>
                          {cell.date.getDate()}
                        </button>
                        <div className="mt-2 space-y-1">
